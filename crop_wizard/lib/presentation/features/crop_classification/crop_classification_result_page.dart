...
class CropClassificationResultPage extends StatelessWidget {
  ...
  const CropClassificationResultPage({super.key, this.imageFile});
    ...
    if (pickedFile != null) {
      ...
      Provider.of<CropClassificationProvider>(context, listen: false).classify(newImageFile);
    } else {
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(AppLocalizations.of(context)!.noImageSelected)), // Or a more specific message
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
        final provider = Provider.of<CropClassificationProvider>(context, listen: false);
        if (provider.state == CropClassificationState.initial && imageFile != null && provider.currentImage == null) {
    ...
    return Scaffold(
      ...
      body: Consumer<CropClassificationProvider>(
        builder: (context, provider, child) {
          final displayImage = provider.currentImage ?? imageFile; // Use provider's current image, fallback to initial
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Text(appLocalizations.classifyingCrop, style: Theme.of(context).textTheme.titleMedium),
                ],
              )
            );
          }
          ...
          if (provider.state == CropClassificationState.success && provider.result != null) {
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  _buildResultCard(context, appLocalizations, [
                    _buildResultRow(context, appLocalizations.cropName, result.crop),
                    _buildResultRow(context, appLocalizations.confidenceScore, '${result.confidenceScore}%'), // Assuming score is a percentage
                    _buildResultRow(context, appLocalizations.stageOfGrowth, result.stageOfGrowth),
                    _buildResultRow(context, appLocalizations.description, result.description, isLast: true),
                  ]),
                ...
                children: [
                  ...
                  const SizedBox(height: 20),
                   ElevatedButton.icon(
                        ...
                        label: Text(appLocalizations.uploadFromGallery), // Or provide a generic "Select Image"
                        onPressed: () => _showRetryImageSourceActionSheet(context),
                    ),
                ],
              )
